...
function setRefreshCookie(res, token) {
  ...
  res.cookie("refresh_token", token, {
    httpOnly: true,
    secure: isProd,
    sameSite: isProd ? "none" : "lax", // cross-site: CloudFront → ALB
...
router.post("/register", async (req, res) => {
  ...
  if (cleanName.length > 64) {
    return res
      .status(400)
      .json({ error: "Name must be at most 64 characters" });
  }
  if (!PHONE_RE.test(cleanPhone)) {
    return res
      .status(400)
      .json({
        error: "Enter a valid mobile number (10–15 digits, optional + prefix)",
      });
  }
